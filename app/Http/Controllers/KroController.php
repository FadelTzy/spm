...

{
    ...
    {
        $data = kro::findorfail($request->id);
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            return Datatables::of(kro::get())->addIndexColumn()->addColumn('aksi', function ($data) {
    ...
    public function destroy($id)
    {
        ...
        $ro = ro::where('id_kro', $id)->get();
        foreach ($ro as $value) {
            $komponen = komponen::where('id_ro', $value->id)->get();
            $value->delete();
            foreach ($komponen as $valuesub) {
                subkomponen::where('id_komponen', $valuesub->id)->delete();
                $valuesub->delete();
            }
        }
        $data->delete();
